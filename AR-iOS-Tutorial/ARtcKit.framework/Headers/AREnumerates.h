...
typedef NS_ENUM(NSInteger, ARErrorCode) {
    ...
    */
    ...
    /** 17: 加入频道被拒绝。
     <p>一般有以下原因:
     <ul><li>用户已进入频道，再次调用加入频道的 API，例如 joinChannelByToken，会返回此错误。停止调用该 API 即可。
     <li>用户在做 Echo 测试时尝试加入频道。等待 Echo test 结束后再加入频道即可。</li></ul></p>
    */
    ARErrorCodeJoinChannelRejected = 17,
    /** 18: 离开频道失败。
     <p>一般有以下原因：
     <ul><li>用户已离开频道，再次调用退出频道的 API，例如 leaveChannel，会返回此错误。停止调用该 API 即可。</li>
     <li>用户尚未加入频道，就调用退出频道的 API。这种情况下无需额外操作。</li></ul></p>
    */
    ...
    /** 109: 109: 当前使用的 Token 过期，不再有效。
     connectionChangedToState 回调中 reason 参数的 ARConnectionChangedTokenExpired(9)。
     <p>一般有以下原因：
     <ul><li>Token 授权时间戳无效：Token 授权时间戳为 Token 生成时的时间戳，自 1970 年 1 月 1 日开始到当前时间的描述。授权该 Token 在生成后的 24 小时内可以访问 anyRTC 服务。如果 24 小时内没有访问，则该 Token 无法再使用。需要重新在服务端申请生成 Token。</li>
     <li>Token 服务到期时间戳已过期：用户设置的服务到期时间戳小于当前时间戳，无法继续使用 anyRTC 服务（比如正在进行的通话会被强制终止）；设置服务到期时间并不意味着 Token 失效，而仅仅用于限制用户使用当前服务的时间。需要重新在服务端申请生成 Token。</li></ul></p>
     */
    ARErrorCodeTokenExpired = 109,
    /** 110: 生成的 Token 无效。
     connectionChangedToState 回调中 reason 参数的 ARConnectionChangedInvalidToken(8)。
     <p>一般有以下原因：
     <ul><li>用户在控制台上启用了 App Certificate，但仍旧在代码里仅使用了 App ID。当启用了 App Certificate，必须使用 Token。</li>
     <li>字段 uid 为生成 Token 的必须字段，用户在调用 joinChannelByToken 加入频道时必须设置相同的 uid。</li></ul></p>
     */
...
typedef NS_ENUM(NSUInteger, ARVideoRenderMode ) {
  /**优先保证视窗被填满。视频尺寸等比缩放，直至整个视窗被视频填满。如果视频长宽与显示窗口不同，则视频流会按照显示视窗的比例进行周边裁剪或图像拉伸后填满视窗。 */
   ARVideoRenderModeHidden = 1,
  /**优先保证视频内容全部显示。视频尺寸等比缩放，直至视频窗口的一边与视窗边框对齐。如果视频尺寸与显示视窗尺寸不一致，在保持长宽比的前提下，将视频进行缩放后填满视窗，缩放后的视频四周会有一圈黑边。 */
...
typedef NS_ENUM(NSInteger, ARAudioProfile) {
    /** 默认设置，通信场景下，默认ARAudioProfileSpeechStandard。直播场景下，默认ARAudioProfileMusicStandard */
    ARAudioProfileDefault = 0,
    /** 指定 32 KHz采样率，语音编码，单声道，编码码率最大值为 18 Kbps */
    ARAudioProfileSpeechStandard = 1,
    /** 指定 48 KHz采样率，音乐编码，单声道，编码码率最大值为 48 Kbps。 */
    ...
    /** 指定 48 KHz采样率，音乐编码，双声道，编码码率最大值为 192 Kbps */
...
typedef NS_ENUM(NSInteger, ARVideoOutputOrientationMode ) {
    /** 自适应布局（默认）
     <p>该模式下SDK输出的视频方向与采集到的视频方向一致。接收端会根据收到的视频旋转信息对视频进行旋转。该模式适用于接收端可以调整视频方向的场景:
     <ul><li>如果采集的视频是横屏模式，则输出的视频也是横屏模式。</li>
     <li>如果采集的视频是竖屏模式，则输出的视频也是竖屏模式。</li></ul></p>
     */
    ARVideoOutputOrientationModeAdaptative = 0,
    /** 横屏布局
     <p>该模式下SDK固定输出人像（横屏）模式的视频。如果采集到的视频是竖屏模式，则视频编码器会对其进行裁剪。该模式适用于当接收端无法调整视频方向时，如使用旁路推流场景下。</p>
     */
    ARVideoOutputOrientationModeFixedLandscape = 1,
    /** 竖屏布局
     <p>该模式下SDK固定输出人像（竖屏）模式的视频。如果采集到的视频是横屏模式，则视频编码器会对其进行裁剪。该模式适用于当接收端无法调整视频方向时，如使用旁路推流场景下。</p>
     */
...
<li>在控制台上启用了 App Certificate，但加入频道未使用 Token。当启用了 App Certificate，必须使用 Token。
<li>在调用 joinChannelByToken 加入频道时指定的 uid 与生成 Token 时传入的 uid 不一致。*/
...
typedef NS_ENUM(NSInteger, ARStreamFallbackOptions ) {
    /** 上行/下行网络较弱时，不对音视频流作回退处理，但不能保证音视频流的质量。 */
    ARStreamFallbackOptionDisabled = 0,
    /** 在下行网络条件较差的情况下，SDK 将接收视频小流（低分辨率、低码率视频s流）。此选项仅适用于 setRemoteSubscribeFallbackOption。 */
